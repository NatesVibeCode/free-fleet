...
class WorkerSession:
    ...
    # Cap retained error text: the count is exact, the tail is diagnostic.
    ...
    def __init__(self, session_id: str, worker_idx: int, route_id: str, provider: str):
        ...
        self.error_count = 0
        self.route_history: list[str] = [route_id]
    ...
    def migrate_route(self, route_id: str, provider: str | None = None) -> bool:
        """Move session affinity to the route that actually verified.

        Returns True when the route changed; every move is appended to
        route_history so lane migration stays auditable.
        """
...
class SessionPool:
    ...
    def get_session(self, session_id: str) -> WorkerSession | None:
