...
def _provider(runner):
    provider = CursorProvider(runner=runner)
    provider._preflight = lambda: None  # type: ignore[method-assign]
    return provider


def test_argv_prompt_is_one_element_with_sandbox():
    runner = RunnerStub(stdout=json.dumps({"result": '{"items": []}'}))
    ok, text, _ = _provider(runner).run_prompt("cursor/some-model", "do the thing")
    assert ok is True
    assert text == '{"items": []}'
    argv = runner.seen["argv"]
    assert isinstance(argv, list) and all(isinstance(part, str) for part in argv)
    assert argv[0] == "cursor-agent"
    assert "--workspace" in argv and "--print" in argv and "--output-format" in argv
